...
void Procedure::replaceQuad(Quad * oldQuad, Quad * newQuad){
	auto itr = std::find(bodyQuads->begin(), bodyQuads->end(), oldQuad);
	itr = bodyQuads->erase(itr);
	bodyQuads->insert(itr, newQuad);
}
