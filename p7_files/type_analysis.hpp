#ifndef DREWGON_TYPE_ANALYSIS
#define DREWGON_TYPE_ANALYSIS
...
public:
	...
	void errAssignFn(const Position * pos){
		hasError = true;
		Report::fatal(pos,
	...
	void errIfCond(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Non-bool expression used as"
			" an if condition");
	}
	void errLoopCond(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Non-bool expression used as"
			" a loop condition");
	...
	void errNotLVal(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Non-Lval assignment");
	}
