...
const DataType * FnTypeNode::getType() const {
	TypeList * inTypes = TypeList::produce(myInTypes);
	const DataType * outType = myOutType->getType();
	return FnType::produce(inTypes, outType);
}
